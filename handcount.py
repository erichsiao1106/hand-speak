...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            mp_drawing.draw_landmarks(frame0, hand_landmarks,mp_hands.HAND_CONNECTIONS,drawing_spec,drawing_spec)
            ...
            for id, lm in enumerate(hand_landmarks.landmark):
                ...
                cv2.circle(frame0, (cx, cy), 8, (38, 107, 170), cv2.FILLED)
                ...
                if id == 3 or id == 4 or id == 8 or id == 12 or id == 16 or id == 20:
                    cv2.circle(frame0, (cx, cy), 5, (0, 0, 255), cv2.FILLED)
                    cv2.putText(frame0,str(id),(cx+8, cy),cv2.FONT_HERSHEY_COMPLEX,.6,(0,0,255),1)
            if len(lmList) != 0:
                fingers = []
                # Thumb
    ...
    cv2.rectangle(frame0, (406, 121), (514, 229), (255, 255, 255), cv2.FILLED)
    cv2.rectangle(frame0, (410, 125), (510, 225), (0, 0, 0), cv2.FILLED)
    if totalFingers==5:
        cv2.putText(frame, str(totalFingers), (30, 210), cv2.FONT_HERSHEY_PLAIN, 6, (255, 0, 0), 8)
        cv2.putText(frame0, str(totalFingers), (430, 210), cv2.FONT_HERSHEY_PLAIN, 6, (255, 0, 0), 8)
    else:
        cv2.putText(frame, str(totalFingers), (30, 210), cv2.FONT_HERSHEY_PLAIN, 6, (0, 0, 255), 8)
        cv2.putText(frame0, str(totalFingers), (430, 210), cv2.FONT_HERSHEY_PLAIN, 6, (0, 0, 255), 8)
    cv2.putText(frame, 'fps:'+str(int(1 / (time.time() - prev_time))), (3, 40), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
    cv2.putText(frame0, 'fps:'+str(int(1 / (time.time() - prev_time))), (403, 40), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255), 3)
